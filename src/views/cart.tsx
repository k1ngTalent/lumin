...
function Cart({ isVisible, setProducts }: IProps) {
    ...
    const { updateCartCurrency, updateCartItemsProductInfo, toggleCartVisibilityState, incrementQuantityOfCartItem, decrementQuantityOfCartItem, deleteCartItem } = useCart();
        ...
        <div className={`cart ${isVisible ? 'visible' : ''}`}>
            <div className="ml-3 d-flex align-items-center pt-3 pb-2 cart-top">
                <div className="d-flex justify-content-start">
                    <div className="cart-toggle rounded-circle border d-flex align-items-center justify-content-center" onClick={()=>{toggleCartVisibilityState(false)}}>
                        <span>&#10148;</span>
                    </div>
                </div>
                <div >
            ...
            <div className="row mx-2">
                <div className="col-lg-3 d-flex justify-content-start">
                    <select className="p-1 my-3 outline-none" onChange={handleCurrencyChange} value={data?.cart?.cartCurrency}>
                        {
                            currencies.data?.currency?.map(
                    ...
                    <div>
                ...
                </div> : <div className="cart-items">
                        {
                            data?.cart?.cartItems?.map((item: any, index: number) => (
                                <CartItem item={item} key={index} currency={data?.cart?.cartCurrency} increment={(id: number) => { incrementQuantityOfCartItem(id) }} decrement={(id: number) => { decrementQuantityOfCartItem(id) }} removeItem={(id: number) => { deleteCartItem(id) }} />
                            ))
                        }
                    </div>
            }
